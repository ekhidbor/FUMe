...
bool validate_da( const std::string& val );
...
public:
    ...
    {
        return std::unique_ptr<value_representation>( new da( *this ) );
    }

protected:
    virtual MC_STATUS validate_value( const std::string& val ) const override final
    {
        return validate_da( val ) ? MC_NORMAL_COMPLETION : MC_INVALID_VALUE_FOR_VR;
    }
