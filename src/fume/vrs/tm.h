...
bool validate_tm( const std::string& val );

// Time value representation
class tm final : public string_vr
{
public:
    tm( unsigned int min_vals, unsigned int max_vals, unsigned int multiple )
    ...
    {
        return std::unique_ptr<value_representation>( new tm( *this ) );
    }

protected:
    virtual MC_STATUS validate_value( const std::string& val ) const override final
    {
        return validate_tm( val ) ? MC_NORMAL_COMPLETION : MC_INVALID_VALUE_FOR_VR;
    }
