...
bool validate_dt( const std::string& val );

// Date Time value representation
class dt final : public string_vr
{
public:
    dt( unsigned int min_vals, unsigned int max_vals, unsigned int multiple )
    ...
    {
        return std::unique_ptr<value_representation>( new dt( *this ) );
    }

protected:
    virtual MC_STATUS validate_value( const std::string& val ) const override final
    {
        return validate_dt( val ) ? MC_NORMAL_COMPLETION : MC_INVALID_VALUE_FOR_VR;
    }
