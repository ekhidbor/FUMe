...
/**
  ...
  be made to the uc_validator.rl file used to produce it.
...
static constexpr size_t MAX_UC_LEN = 0xFFFFFFFEu;

%% write data;

bool validate_uc( const string& val )
{
    bool ok = false;

    if( val.size() <= MAX_UC_LEN )
    {
        int cs = 0;
