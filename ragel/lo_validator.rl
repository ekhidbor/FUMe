...
/**
  ...
  be made to the lo_validator.rl file used to produce it.
...
static constexpr size_t MAX_LO_LEN = 64u;

%% write data;

bool validate_lo( const string& val )
{
    bool ok = false;

    if( val.size() <= MAX_LO_LEN )
    {
        int cs = 0;
