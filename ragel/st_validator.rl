...
/**
  ...
  be made to the st_validator.rl file used to produce it.
...
static constexpr size_t MAX_ST_LEN = 1024u;

%% write data;

bool validate_st( const string& val )
{
    bool ok = false;

    if( val.size() <= MAX_ST_LEN )
    {
        int cs = 0;
